...
export const sendResetPasswordEmail = async (to: string, otp: string): Promise<void> => {
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e4e4e7; border-radius: 12px;">
      ...
      <h2 style="color: #FF5A5F; text-align: center; margin-bottom: 20px;">Password Reset OTP</h2>
      <p style="color: #3f3f46; font-size: 16px; line-height: 1.5;">Hello,</p>
      <p style="color: #3f3f46; font-size: 16px; line-height: 1.5;">We received a request to reset the password for your Swigo account. Please enter the following One-Time Password (OTP) to complete your request:</p>
      <div style="background-color: #f4f4f5; padding: 15px; text-align: center; font-size: 32px; font-weight: bold; letter-spacing: 8px; color: #18181b; border-radius: 8px; margin: 25px auto; max-width: 300px; border: 1px solid #e4e4e7;">
        ${otp}
      </div>
      <p style="color: #3f3f46; font-size: 14px; line-height: 1.5;">This OTP is valid for <strong>15 minutes</strong>. If you did not request a password reset, you can safely ignore this email and your password will remain secure.</p>
