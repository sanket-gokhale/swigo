...
interface LocationData {
  latitude: number;
  longitude: number;
  address?: string; // We can add geocoding later if needed
}
...
export const LocationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const saveLocation = async (lat: number, lng: number) => {
    ...
    try {
      ...
      // Extract shorter address if possible (e.g., City, Country)
      if (data.address) {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        if (err.code === 1) {
          ...
          // Timeout - usually happens if user ignores the prompt
          console.warn('Geolocation: Request timed out (user likely ignored prompt)');
        } else {
  ...
  useEffect(() => {
    ...
    if (storedLocation) {
      ...
      // Auto request on mount as per user flow
      requestLocation();
    }
  }, []);
