        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          <div className="space-y-6">
            ...
            <p className="text-zinc-400 text-sm font-bold leading-relaxed max-w-xs">
              Connecting home kitchens and food providers directly with tenants, PGs, and local customers.
            </p>
          </div>

          <div>
            <h4 className="text-xs font-black uppercase tracking-widest text-zinc-500 mb-6">Kitchen Portal</h4>
            <ul className="space-y-3">
              {[
                { name: 'Dashboard', path: '/dashboard' },
                { name: 'Find Properties', path: '/find-properties' },
                { name: 'Food Requests', path: '/requests' },
                { name: 'Partnerships', path: '/collabs' },
                { name: 'Profile & Photos', path: '/profile' },
              ].map(item => (
                <li key={item.name}>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {['Help Center', 'Food Safety Guidelines', 'Terms of Service', 'Privacy Policy'].map(item => (
                <li key={item}>
